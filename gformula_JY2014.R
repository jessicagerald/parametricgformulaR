...
gformula_pool_sim <- function(trt){
  ...
  for (jj in 1:N){
    ...
    for (kk in 1:Ndat){
      if (kk==1){
        L<-lag.L<-Py<-prodp0<-prodp1<-prodp2<-pL<-NULL
        L[kk]<-subj$L[kk]
        lag.A[kk] <-pL[kk]<-0
        # d0<-cbind(L[kk],Astar[kk],lag.A[kk])
        # colnames(d0)<-c("L","A","lag.A")
        d0<-data.frame(L=L[kk],A=Astar[kk],lag.A=lag.A[kk])
        Py[kk]<-predict(fitY.1,type="response",newdata=data.frame(d0)) ##gives the predicted probability
        prodp0[kk]<- 1-Py[kk]
        prodp1[kk]<- Py[kk]
      }
      else if (kk>1){
        # d1<-cbind(lag.A[kk])
        # colnames(d1)<-c("lag.A")
        d1<-data.frame(lag.A=lag.A[kk])
        pL[kk]<-predict(fitL.1,type="response",newdata=data.frame(d1))
    ...
    pool<- rbind(pool,cbind(id=jj,t0,Astar,L,lag.A,Py,prodp0,prodp1,poprisk,pL))
  }
  return(pool)
}
...
for (i in 1:Bsample){
  ...
  setkey(test1, "id")
  sample <- test1[J(ids), allow.cartesian = TRUE]  # create the new data set names "sample"
  sample$id<-NULL
  colnames(sample)[colnames(sample)=="newid"]<-"id"
